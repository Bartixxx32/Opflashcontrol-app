...
import com.bartixxx.opflashcontrol.BaseActivity.Companion.WHITE2_LED_PATH
import com.bartixxx.opflashcontrol.BaseActivity.Companion.YELLOW2_LED_PATH
...
class LEDControlTileService : TileService() {
    ...
    private val brightnessSteps = listOf(10, 30, 50, 80, 100, 120, 150, 180, 210, 240, 255) // Defined brightness steps
        ...
        try {
            if (currentTime - lastTapTime < doubleTapInterval) {
                ...
                if (currentBrightnessState == 0 || currentBrightnessState == 1) {
                    ...
                    currentBrightnessState = brightnessSteps.indexOf(80) + 1 // Set to 80% brightness
                    controlAllLeds(80)
                    updateTileDescription()
                } else {
                ...
                currentBrightnessState = (currentBrightnessState + 1) % (brightnessSteps.size + 1) // Includes 0 (off)
                updateTileDescription()
                ...
                val brightnessLevel = if (currentBrightnessState == 0) 0 else brightnessSteps[currentBrightnessState - 1]
    ...
    private fun updateTileDescription() {
        try {
            ...
            qsTile.label = if (currentBrightnessState == 0) getString(R.string.flashlight_off) else getString(R.string.flashlight_on)
